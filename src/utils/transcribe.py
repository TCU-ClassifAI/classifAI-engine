...
def start_transcription():
    ...
    if audio_file:
        ...
        # grab the title of the audio file
        title = request.form.get("title")

        # Create job in job_status
        job_status[uuid_audio_file] = TranscriptionJob(
            uuid_audio_file, model_type=model_name, title=title
        )
    ...
    try:
        ...
        if language in wav2vec2_langs:
            ...
            for segment in whisper_results:
                ...
                # add avg_logprob for each segment
                word_timestamps[-1]["avg_logprob"] = segment["avg_logprob"]
        ...
        ssm_objects = list(
            map(
                lambda x: TranscriptionSegment(
                    ...
                    end_time=x["end_time"],
                    text=x["text"],
                    confidence=x.get("avg_logprob", 1.0),
                ),
                ssm,
            )
        )
        ...
        if job_id is not None:
            ...
            job_status[job_id].duration = job_status[job_id].get_duration()
